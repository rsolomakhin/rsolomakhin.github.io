 ...
 */
async function buildPaymentRequest(windowLocalStorageIdentifier, payeeName, payeeOrigin) {
...
async function onBuyClicked(windowLocalStorageIdentifier, payeeName, payeeOrigin) {
  ...
  const request = await buildPaymentRequest(windowLocalStorageIdentifier, payeeName, payeeOrigin);
  if (!request) return;
  try {
