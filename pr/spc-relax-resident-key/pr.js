  ...
  if (window.PaymentCredential) {
    ...
    const publicKey = {
      ...
      extensions: {
        ...
        // Specify credProps so that the authenticator returns the type of
        // credential created. 
        credProps: true,
      }
    };
...
function isDiscoverable(publicKeyCredential) {
...
async function createPaymentCredential(windowLocalStorageIdentifier) {
  try {
    ...
    info(windowLocalStorageIdentifier + ' enrolled: ' + objectToString(
      publicKeyCredential));
    info('Discoverable credential created? ' + isDiscoverable(publicKeyCredential));
  } catch (err) {
    error(err);
  }
}
